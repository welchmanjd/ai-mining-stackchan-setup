...
3. デバイスマネージャーで接続先（COMポート）が見えるか
4. Arduino IDE等の他アプリがポートを掴んでいないか
5. USBハブではなくPC本体ポートに直挿しか

### 書き込みに失敗しました
1. USBを抜き差しして、もう一度ためす
2. 別のUSBポート/ケーブルで、もう一度ためす
3. M5Stack Core2を再起動して、もう一度ためす
