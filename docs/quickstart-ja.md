...
このページは、はじめての人向けです。上から順番に進めれば、初期設定ができます。
...
3. ZIPを右クリックして展開する（`右クリック -> すべて展開`）
4. 展開したフォルダ内の `AiStackchanSetup.bat` をダブルクリックする
5. M5Stack Core2 をUSBでPCにつなぐ
...
動画版ガイド（YouTube）は準備中です。公開後、このページにURLを追加します。
